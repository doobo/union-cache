...
public class ZipUtils {
	...
	public static String gzip(String primStr) {
		if (primStr == null || primStr.length() == 0) {
	 	...
	 	log.debug("压缩前占用空间："+primStr.getBytes().length);
		...
		log.debug("压缩后占用空间："+gzipStr.getBytes().length);
		return gzipStr;
	}
	...
	public static final String zip(String str) {
		...
		log.debug("压缩前占用空间："+str.getBytes().length);
		byte[] compressed;
		...
		log.debug("压缩后占用空间："+compressedStr.getBytes().length);
